...
## 2024-05-27 - Missing Default Security Headers
**Vulnerability:** The application was missing basic security headers such as `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, and `Strict-Transport-Security`, potentially exposing users to clickjacking, MIME-type sniffing, and man-in-the-middle attacks.
**Learning:** Default Next.js applications do not automatically configure these headers. Relying entirely on framework defaults leaves applications vulnerable to basic attacks.
**Prevention:** Always explicitly define security headers using the `async headers()` configuration block in `next.config.ts`.

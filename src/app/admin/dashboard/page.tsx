...
export default function AdminDashboardPage() {
  ...
  if (loading) return (
    ...
    <main className="min-h-screen flex items-center justify-center bg-[var(--color-background)] text-[var(--color-foreground)]">
      <h1 className="text-3xl font-bold mb-4 text-primary-700">Admin Dashboard</h1>
    ...
    <main className="min-h-screen bg-[var(--color-background)] text-[var(--color-foreground)] py-10 px-2 sm:px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-4xl font-extrabold text-primary-700 tracking-tight drop-shadow-lg">Admin Dashboard</h1>
          <div className="space-x-4 flex items-center flex-wrap gap-y-2">
            ...
            <a href="/admin/dashboard/settings" className="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 transition">Site Settings</a>
          </div>
        </div>
        {/* Responsive Table for Desktop, Cards for Mobile */}
        <div className="hidden md:block">
          {/* Updated table container styles */}
          <div className="overflow-x-auto rounded-xl shadow-lg bg-white dark:bg-gray-800 border border-primary-100 dark:border-gray-700">
            {/* Updated table styles */}
            <table className="min-w-full divide-y divide-primary-100">
              {/* Updated table header styles */}
              <thead className="sticky top-0 z-10 bg-primary-50 backdrop-blur border-b border-primary-200">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-primary-800 uppercase tracking-wider">Item</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-primary-800 uppercase tracking-wider">Price</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-primary-800 uppercase tracking-wider">Claimed/Funded</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-primary-800 uppercase tracking-wider">Contributions</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-primary-800 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              {/* Updated table body styles */}
              <tbody className="divide-y divide-primary-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-primary-50/50 transition">
                    ...
                    <td className="px-4 py-3 space-x-2">
                      ...
                      <button
                        ...
                        className="bg-primary-600 hover:bg-primary-700 text-white px-3 py-1 rounded text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-primary-300"
                        onClick={async () => {
        ...
        <div className="md:hidden space-y-6">
          {items.map((item) => (
            // Updated card styles
            <div key={item.id} className="rounded-xl shadow-lg bg-white dark:bg-gray-800 p-4 flex flex-col gap-2 border border-primary-100 dark:border-gray-700">
              <div className="flex justify-between items-center mb-2">
                {/* Updated item name color */}
                <span className="font-bold text-lg text-primary-700">{item.name}</span>
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button
                  // Updated Delete button (rose)
                  className="flex-1 bg-primary-600 hover:bg-primary-700 text-white px-3 py-2 rounded text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-primary-300"
                  onClick={async () => {
                    if (!confirm('Are you sure you want to delete this item?')) return;
                    try {
                      const res = await fetch(`/api/registry/items/${item.id}`, { method: 'DELETE' });
        ...
        <div className="mt-10 flex justify-center">
          <button
            ...
            className="bg-gradient-to-r from-primary-700 to-amber-500 hover:from-amber-500 hover:to-primary-700 text-white px-6 py-3 rounded-lg text-base font-semibold shadow-lg focus:outline-none focus:ring-2 focus:ring-primary-300"

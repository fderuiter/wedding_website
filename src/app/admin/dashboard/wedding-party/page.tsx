    ...
    >
      <div className="py-10 px-4 sm:px-6 max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-extrabold text-primary-700">Wedding Party Studio</h1>
          <div>
            ...
            <button onClick={() => { 
              ...
              setIsEditing(true); 
            }} className="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 transition">Add New Member</button>
          </div>
        </div>
        ...
        <div className="grid gap-4 pb-10">
          {members.map(member => (
            <div key={member.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow border border-primary-100 flex justify-between items-center">
              <div className="flex items-center gap-4">
                {member.photo && <img src={member.photo} alt={member.name} className="w-16 h-16 rounded-full object-cover" />}
                <div>
                  <div className="font-bold text-lg">{member.name}</div>
                  <div className="text-sm font-semibold text-primary-600">{member.role}</div>
              ...
              <div className="space-x-2 flex flex-col gap-2">
                ...
                <button onClick={() => handleDelete(member.id)} className="bg-primary-600 text-white px-3 py-1 rounded text-sm">Delete</button>
              </div>
            </div>
          ))}

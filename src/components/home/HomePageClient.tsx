...
export default function HomePageClient({ calendarEvent, config: initialConfig, contentNodes: initialContentNodes = [] }: { calendarEvent: CalendarEvent, config: AppConfig, contentNodes?: ContentNode[] }) {
  ...
  const renderSection = (feature: any, index: number) => {
    ...
    switch (feature.type) {
      case 'story':
        return (
          <motion.section key={feature.id} id={feature.id} className="mx-auto max-w-3xl space-y-8 px-4 py-20 sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-center text-4xl font-bold text-primary-700">{feature.title || 'Our Story'}</h2>
      ...
      case 'details':
        return (
          <motion.section key={feature.id} id={feature.id} className="px-4 py-20 sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-center text-4xl font-bold text-primary-700 mb-10">{feature.title || 'Wedding Day Details'}</h2>
            <div className="mx-auto grid max-w-5xl gap-10 md:grid-cols-2">
              <div className="rounded-2xl border border-primary-100 dark:border-primary-700 bg-white dark:bg-gray-800 p-8 shadow-lg transition-transform hover:scale-[1.02]">
      ...
      case 'accommodations':
        return (
          <motion.section key={feature.id} id={feature.id} className="mx-auto max-w-3xl space-y-8 px-4 py-20 text-center sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-4xl font-bold text-primary-700">{feature.title || `Accommodations in ${config.venueCity}, ${config.venueState}`}</h2>
      ...
      case 'venue':
        return (
          <motion.section key={feature.id} id={feature.id} className="mx-auto max-w-3xl space-y-8 px-4 py-20 text-center sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-4xl font-bold text-primary-700">{feature.title || 'About Our Venue'}</h2>
      ...
      case 'travel':
        return (
          <motion.section key={feature.id} id={feature.id} className="mx-auto max-w-3xl space-y-8 px-4 py-20 text-center sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-4xl font-bold text-primary-700">{feature.title || `Travel & Things to Do in ${config.venueCity}`}</h2>
            {logisticsNodes.length > 0 ? (
              logisticsNodes.map((node, i) => (
                <div key={i} className="text-left bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mt-4 border border-primary-100 dark:border-primary-700">
                  {node.title && <h3 className="font-semibold text-xl mb-2 text-primary-700">{node.title}</h3>}
      ...
      case 'faq':
        return (
          <motion.section key={feature.id} id={feature.id} className="mx-auto max-w-3xl space-y-8 px-4 py-20 sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-center text-4xl font-bold text-primary-700">{feature.title || 'Questions You Probably Have'}</h2>
      ...
      case 'custom':
        return (
          <motion.section key={feature.id} id={feature.id} className="mx-auto max-w-3xl space-y-8 px-4 py-20 sm:px-6 lg:px-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={index * 0.1}>
            <h2 className="text-center text-4xl font-bold text-primary-700">{feature.title}</h2>
  ...
  return (
    <>
      ...
      */}
      <div id="top" />
      <div className="min-h-screen bg-[var(--color-background)] text-[var(--color-foreground)] selection:bg-primary-100 selection:text-primary-900 dark:selection:bg-primary-800"
      >
        <main id="main">
          <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 py-28 text-center sm:px-6 lg:px-8">
            ...
            <motion.h1 className="mb-6 text-5xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-primary-700 to-amber-500 sm:text-6xl lg:text-7xl" variants={fadeUp} initial="hidden" animate="visible" custom={0}>
            ...
            <motion.div className="flex flex-col items-center gap-3 sm:flex-row sm:gap-6" variants={fadeUp} initial="hidden" animate="visible" custom={2}>
              <a href="#story" className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-primary-700 to-amber-500 px-8 py-3 text-white shadow-lg transition hover:shadow-xl">
                Our Story
          ...
          <footer className="flex flex-col items-center gap-4 px-4 pb-10 text-sm text-gray-500 dark:text-gray-400">
            ...
            <a href="/project-info" className="text-primary-600 dark:text-primary-400 hover:underline">About this site</a>
            <Link
              href="/heart"
              className="inline-block rounded-full bg-gradient-to-r from-amber-500 to-primary-700 px-10 py-4 font-medium text-white shadow-lg transition hover:scale-105 hover:shadow-xl"
            >
              Play with the Heart
            </Link>
          </footer>
